...
function startUp(){
	/*insert the title for puzzle 1*/
	document.getElementById("puzzleTitle").innerHTML = "puzzle1";
	
	/*insert the HTML code for puzzle 1*/
...
function findErrors(){
	for(var k = 0; k < allCells.length; k++){
		if((allCells[k].className == "blocks" && allCells[k].style.backgroundColor == "rgb(101, 101, 101)") || (allCells[k].className == "circles" && allCells[k].style.backgroundColor == "black")){
			allCells[k].style.color = "red";
		}
	}
	setTimeout(function () {
		for(var k = 0; k < allCells.length; k++){
			if(allCells[k].style.color == "red"){
				allCells[k].style.color = "white"
			}
		}
	}, 1000)
}
